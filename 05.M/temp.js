...
function restoreFromStorage(){
  try{
    ...
    toast(`📂 ${S.md.length}개 문서 복원됨`,'inf');
    return true;
  }catch(e){console.warn('Storage restore failed:',e);return false;}
}

function clearStorage(){
  if(!confirm('저장된 모든 문서를 삭제할까요?\n(Raw 파일 + 변환된 MD + 수정 기록 전체)')) return;
...
function copyMdToClipboard(){
  ...
  navigator.clipboard.writeText(S.activeDoc.content)
    .then(()=>toast('📋 Markdown 클립보드에 복사됨','ok'))
    .catch(()=>toast('복사 실패 (권한 필요)','err'));
}
...
function downloadAllMd(){
  ...
  URL.revokeObjectURL(url);
  toast(`⬇️ ${S.md.length}개 문서 번들 다운로드됨`,'ok');
}
